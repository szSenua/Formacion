...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_stmt_execute($stmtCerrarCurso)) {
        ...
        $sqlSolicitudes = "SELECT DISTINCT s.*, c.nombre as nombre_curso, c.numeroplazas,
                           ...
                           ORDER BY s.admitido ASC, prioridad ASC, r.puntos DESC, (SELECT COUNT(*) FROM solicitudes s3 WHERE s3.dni = s.dni AND s3.admitido = 1) ASC";
        ...
        foreach ($solicitudesArray as $solicitud) {
            ...
            if ($plazasDisponibles > 0) {
                // Obtener el número de solicitudes admitidas del solicitante
                $sqlSolicitudesAdmitidas = "SELECT COUNT(*) as num_admitidas FROM solicitudes WHERE dni = ? AND admitido = 1";
                $stmtSolicitudesAdmitidas = mysqli_prepare($conexion, $sqlSolicitudesAdmitidas);
                mysqli_stmt_bind_param($stmtSolicitudesAdmitidas, "s", $dniSolicitante);
                mysqli_stmt_execute($stmtSolicitudesAdmitidas);
                $resultSolicitudesAdmitidas = mysqli_stmt_get_result($stmtSolicitudesAdmitidas);
                $rowSolicitudesAdmitidas = mysqli_fetch_assoc($resultSolicitudesAdmitidas);
                $numSolicitudesAdmitidas = $rowSolicitudesAdmitidas['num_admitidas'];

                // Verificar si el solicitante tiene solicitudes admitidas
                if ($numSolicitudesAdmitidas == 0) {
                    // Si no tiene solicitudes admitidas, se le asigna automáticamente y se actualiza la base de datos
